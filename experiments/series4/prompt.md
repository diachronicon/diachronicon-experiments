...
Analyze the following constuction:

### Research Algorithm

1. Run 1–3 queries to Russian National Corpus covering the most frequent surface forms of the construction. Examine the **first** and the **last page** of each result set. If a query returns more than 20 pages, do not read sequentially: sample pages at regular intervals to capture all possible diachronic changes of the given construction. **Samples set does not have to exceed 800 items**.
2. Run additional queries only if samples reveal:
  * a suspected variation not yet confirmed by 3 examples;
  * a chronological gap of more than 30 years with no attestations;
  * a specific syntactic or semantic property that needs verification.
Each follow-up query must have an explicit stated goal before it is run.
3. **Look through the resulting samples** to develop a comprehensive understanding of the historical development of the expression (specifically focusing on constructionalization and grammaticalization). Always make sure to look through all the available pages to get a more representative result.
4. **Check for homonymy.** Before proceeding, determine whether the target construction is formally homonymous with another one. If so, establish explicit syntactic and semantic criteria to distinguish them, and document this disambiguation. Only then analyze the target construction separately.
5. **Analyze both syntactic and semantic properties** during tracking. The expression must remain persistent and recognizable throughout its changes.
6. **Produce a detailed numbered list** of all significant syntactic and semantic changes that occurred during the construction's development and formed its historical variations.
  * The main list starts with the source stage, indicating whether corpus reveals information on gradual constructionalization of the construction or contain one in the idiomatic form. **The source stage has to be presented once.**
  * A variation is considered **occasional** and belongs in the special section before the final table if it is attested by fewer than 3 examples in the corpus.
  * If an example employs the construction as material for wordplay (pun, foregrounding of literal meaning), it does **not** constitute evidence of a change. Place it in the special section before the final table.
  * If a construction permits negation via a standard negative particle ("не") without any modification to its internal structure, argument frame, or meaning — this does not constitute a separate stage. Place such cases in the special section before the final table.
7. **Formalize the description** for every particular variation using the following structure:
  ...
  * Identify and include both the **first** and the **last** chronological entries for that specific variation, illustrating the described linguistic characteristics. Last entry must be included in all cases! Comment on the choice.
8. **Reorganize the list of changes** into the most logical and linguistically sound order in which the changes could have taken place.
9. **Identify the relationship** at the beginning of each change description: mark whether the variation succeeds the previous one or is concurrent with it. Pay special attention to the variation numbering.

Report the results by filling the variation logging form for each variation, section for special cases; summarize the results by filling the final table form (see both below).

### Strict Constraints

* **ONLY USE WHAT IS GIVEN IN THE CORPORA.**
* **DO NOT hallucinate.** 
* **DO NOT make up examples or dates**.
...
* **Each variation describes exactly one change.** If syntactic and semantic changes occurred simultaneously, treat them as two separate stages.
* If you are unsure about the order, return several variants of the analysis rather than iterating over the data several times over.
