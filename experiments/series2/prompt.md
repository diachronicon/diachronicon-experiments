...
2. **Look through the resulting samples** to develop a comprehensive understanding of the historical development of the expression (specifically focusing on constructionalization and grammaticalization). Always make sure to look through all the available pages to get a more representative result.
...
7. **Identify the relationship** at the beginning of each change description: mark whether the variation succeeds the previous one or is concurrent with it. Pay special attention to the variation numbering.
